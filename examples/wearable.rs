...
mod wheel {
    ...
    use propertybindings::items::{DrawResult, Geometry, Item, LayoutInfo, MouseEvent};
    ...
    impl<'a> WheelLayout<'a> {
        ...
        fn build_layout(this: &Rc<Self>) {
            ...
            for (idx, x) in this.children.borrow().iter().enumerate() {
                ...
                x.geometry()
                    .width
                    .set_binding(Some(move || Some(w.upgrade()?.item_size.get())));
                let w = Rc::downgrade(this);
                x.geometry()
                    .height
                    .set_binding(Some(move || Some(w.upgrade()?.item_size.get())));
                ...
                x.geometry().x.set_binding(Some(move || {
                    Some({
                        let w = w.upgrade()?;
                        w.geometry().width() / 2.
                            + w.geometry().width() / 3. * (a + w.angle.get()).cos()
                            - w.item_size.get() / 2.
                    })
                }));
                let w = Rc::downgrade(this);
                x.geometry().y.set_binding(Some(move || {
                    Some({
                        let w = w.upgrade()?;
                        w.geometry().height() / 2.
                            + w.geometry().height() / 3. * (a + w.angle.get()).sin()
                            - w.item_size.get() / 2.
                    })
                }));
            }
