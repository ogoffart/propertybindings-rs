...
impl<'a, F: PropertyBindingFn<f64> + 'a> AnchorApplyGeometry<'a>
    for AnchorElement<'a, BeginTag, F>
{
...
impl<'a, F: PropertyBindingFn<f64> + 'a> AnchorApplyGeometry<'a>
    for AnchorElement<'a, CenterTag, F>
{
    fn apply_geometry(self, begin: &Property<'a, f64>, size: &Property<'a, f64>) {
        let ws = size.as_weak();
...
impl<'a, F1: PropertyBindingFn<f64> + 'a, F2: PropertyBindingFn<f64> + 'a> AnchorApplyGeometry<'a>
    for (
        AnchorElement<'a, BeginTag, F1>,
        AnchorElement<'a, EndTag, F2>,
    )
{
    fn apply_geometry(self, begin: &Property<'a, f64>, size: &Property<'a, f64>) {
...
impl<'a, F1: PropertyBindingFn<f64> + Clone + 'a, F2: PropertyBindingFn<f64> + Clone + 'a>
    AnchorApplyGeometry<'a>
    for (
        AnchorElement<'a, EndTag, F1>,
        AnchorElement<'a, CenterTag, F2>,
    )
{
    fn apply_geometry(self, begin: &Property<'a, f64>, size: &Property<'a, f64>) {
        ...
        begin.set_binding(Some(move || {
            Some(2. * (self.1).f.run()? - (self.0).f.run()?)
        }));
