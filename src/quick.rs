...
impl Application {
    ...
    pub fn get_callback<F: FnOnce() + Send + 'static>(&self) -> impl Fn(F) + Send {
        ...
        move |callback| {
            proxy
                .send_event(UserEvent::User(Box::new(callback)))
                .unwrap_or_else(|_| panic!("Could not send the event"));
        }
    }
    ...
    pub fn show_window<T: ItemFactory + 'static>(self) {
        ...
        let mut state = ApplicationState {
            cursor_pos: piet_common::kurbo::Point::ORIGIN,
        };
            ...
            if let Some(image_index) = sw_window.poll_next_image() {
                ...
                let raw_pixels = bitmap.into_raw_pixels(ImageFormat::RgbaPremul).unwrap();
                {
                    ...
                    for y in 0..(height as usize) {
                        (*surface)[y * stride..(y * stride + (4 * width as usize))]
                            .copy_from_slice(
                                &raw_pixels
                                    [y * (width as usize) * 4..(y + 1) * (width as usize) * 4],
                            );
                    }
                }
